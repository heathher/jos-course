...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
	...
	//LAB 3: Your code here.
	 e->env_tf.tf_esp = 0x210000 + 0x4096 * 2 * (e - envs);
#else
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	int success = 0;
	void* begin = (void*)ROUNDDOWN((uintptr_t)va, PGSIZE), *end = (void*)ROUNDUP((uintptr_t)(va) + len, PGSIZE);
	void* i;
	for (i = begin; i < end; i += PGSIZE){
		...
		success = page_insert(e->env_pgdir, page, i, PTE_U | PTE_W | PTE_P);
		if (success == -E_NO_MEM){
			panic("region alloc: error in page_insert!\n");
		}
	}
...
bind_functions(struct Env *e, struct Elf *elf)
{
	...
	//LAB 3: Your code her 
	struct Secthdr *table_start = (struct Secthdr *) ((uint8_t *) elf + elf->e_shoff);
	struct Secthdr *table_end = table_start + elf->e_shnum;
	struct Secthdr *sh;
	char *sh_strtab = (char *) elf + table_start[elf->e_shstrndx].sh_offset;
	char *strtab = NULL;
	struct Elf32_Sym *sym_start = NULL, *sym_end = NULL, *sym;
	uintptr_t addr;
	for (sh = table_start; sh < table_end; sh++) {
		if (!memcmp(&sh_strtab[sh->sh_name], ".strtab", strlen(".strtab")))
			strtab = (char *) elf + sh->sh_offset;
		if (!memcmp(&sh_strtab[sh->sh_name], ".symtab", strlen(".symtab"))) {
	...
	for (sym = sym_start; sym < sym_end; sym++) {
		if ((addr = find_function(&strtab[sym->st_name]))) {
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	if (ELFHDR->e_magic != ELF_MAGIC)
		panic("load_icode: not elf_magic\n");
	...
	for (; ph < eph; ph++){
		if (ph->p_memsz < ph->p_filesz){
			panic("load_icode: ph->p_filesz is too big!\n");
		}
		if (ph->p_type == ELF_PROG_LOAD && ph->p_filesz <= ph->p_memsz){
			//for lab8
			region_alloc(e, (void *)ph->p_va, ph->p_memsz);
			//
			memmove((void *)ph->p_va, binary + ph->p_offset, ph->p_filesz);              //copy 
			memset((void *)ph->p_va + ph->p_filesz, 0, ph->p_memsz-ph->p_filesz);        //cleared to zero
		}			
	}
	e->env_tf.tf_eip = ELFHDR->e_entry;                                                  //program's entry program
